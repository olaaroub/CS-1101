...
t_bsq* getData(char* input){
	...
	if(!parseLine(data, file))
		return(fclose(file), freeData(data), NULL);
	...
	for (int i = 0; i < data->lines; i++)
	{
		...
		len = getline(&line, &n, file);
		if(len<1 || line[len-1] != '\n')
			return(printf("DDDD\n"),fclose(file), free(line), freeData(data), NULL);
		line[len-1]='\0';
		if(i==0)
			data->lenght = len -1;
		if(data->lenght != (int)len-1)
			return(fclose(file), free(line), freeData(data), NULL);
		data->map[i] = line;
	}
...
bool checkData(t_bsq* data){
	...
	for (int i = 0; i < data->lines; i++)
	{
		...
		{
			if(data->map[i][j] != data->obstacle && data->map[i][j] != data->empty)
				return(freeData(data), false);
		}
	}
	return true;
